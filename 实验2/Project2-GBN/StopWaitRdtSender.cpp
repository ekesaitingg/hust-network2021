...
bool StopWaitRdtSender::send(const Message &message) {
	...
	if (nextseqnum < (base + winsize)) {
		...
		printf("当前发送窗口为[%d,%d]", base, base+3);
...
void StopWaitRdtSender::receive(const Packet& ackPkt) {
	...
	if (checkSum == ackPkt.checksum && ackPkt.acknum >= this->packetWaitingAck.seqnum) {
		...
		head = (head + ackPkt.acknum-this->packetWaitingAck.seqnum+1) % 10;
...
void StopWaitRdtSender::timeoutHandler(int seqNum) {
		...
		for (int i = 0; i < this->nextseqnum - this->base; i++) {
			packetsending = packet[head+i];
